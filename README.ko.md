...
- **웹 인터페이스** — 브라우저에서 히스토리를 조회·검색·관리하는 UI
...
| 명령어 | 설명 |
|--------|------|
| `clipd web` | `http://localhost:8432`에서 웹 UI 시작 (브라우저 자동 열림) |
| `clipd web --port 9000` | 커스텀 포트 사용 |
| `clipd web --no-open` | 브라우저 자동 열기 없이 시작 |

웹 인터페이스에서 히스토리 조회, 검색, 무한 스크롤, 복사, 핀/태그, 삭제, 내보내기를 모두 지원합니다 — 터미널 없이 사용 가능.
